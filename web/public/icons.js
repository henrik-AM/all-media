// Stroke icon set (feather-style). No emoji anywhere in the UI chrome.
const ICONS = (() => {
  const wrap = (body, opts = {}) =>
    `<svg viewBox="0 0 24 24" fill="${opts.fill || 'none'}" stroke="currentColor" stroke-width="${opts.sw || 1.8}" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${body}</svg>`;

  return {
    play: wrap('<path d="M4.5 4.9v14.2a1 1 0 0 0 1.53.85l11.2-7.1a1 1 0 0 0 0-1.7L6.03 4.05A1 1 0 0 0 4.5 4.9z"/>'),
    chat: wrap('<path d="M21 11.5a8.4 8.4 0 0 1-9 8.4 9 9 0 0 1-3.6-.8L3 21l1.9-5.1A8.4 8.4 0 0 1 12 3.1a8.4 8.4 0 0 1 9 8.4z"/>'),
    camera: wrap('<path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h3.5L8 3h8l1.5 3H21a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/>'),
    person: wrap('<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>'),
    people: wrap('<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/>'),
    /* Diagonaler Doppelpfeil - oeffnet die Karte im Vollbild und schliesst
       sie wieder. Henrik wollte ihn anstelle von Plus/Minus. */
    ausklappen: wrap('<path d="M15 3h6v6"/><path d="M9 21H3v-6"/><path d="M21 3l-7.5 7.5"/><path d="M3 21l7.5-7.5"/>'),
    einklappen: wrap('<path d="M4 14h6v6"/><path d="M20 10h-6V4"/><path d="M14 10l7-7"/><path d="M3 21l7-7"/>'),
    /* Gestapelte Ebenen - Umschalter fuer die Kartenansicht. */
    ebenen: wrap('<path d="M12 2 2 7l10 5 10-5-10-5z"/><path d="m2 17 10 5 10-5"/><path d="m2 12 10 5 10-5"/>'),
    /* Drei Striche. Der dritte Knopf oben im eigenen Profil - im Prototyp
       (Frame "Community - Profil") steht dort ein Menue, kein Zahnrad. Die
       App zeigte es richtig, die Website nicht. */
    menu: wrap('<path d="M3 6h18M3 12h18M3 18h18"/>'),
    search: wrap('<circle cx="11" cy="11" r="7"/><path d="m20 20-3.2-3.2"/>'),
    plus: wrap('<path d="M12 5v14M5 12h14"/>'),
    back: wrap('<path d="M19 12H5M12 19l-7-7 7-7"/>'),
    close: wrap('<path d="M18 6 6 18M6 6l12 12"/>'),
    /* QR-Code und Scan-Rahmen — Kontakt hinzufuegen, siehe openAddContact. */
    qr: wrap('<rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><path d="M14 14h3v3h-3zM20 14h1M14 20h1M18 18h3v3h-3z"/>'),
    scan: wrap('<path d="M3 8V5a2 2 0 0 1 2-2h3M16 3h3a2 2 0 0 1 2 2v3M21 16v3a2 2 0 0 1-2 2h-3M8 21H5a2 2 0 0 1-2-2v-3M3 12h18"/>'),
    minus: wrap('<path d="M5 12h14"/>'),
    pause: wrap('<path d="M9 5v14M15 5v14"/>', { sw: 2.2 }),
    /* Lautsprecher fuer den Reel-Kanal. Reels starten stumm — Browser
       lassen eine Wiedergabe ohne Zutun des Nutzers nur ohne Ton zu. */
    tonAus: wrap('<path d="M11 5 6 9H2v6h4l5 4z"/><path d="M22 9l-6 6M16 9l6 6"/>'),
    tonAn: wrap('<path d="M11 5 6 9H2v6h4l5 4z"/><path d="M19.1 4.9a10 10 0 0 1 0 14.2M15.5 8.5a5 5 0 0 1 0 7"/>'),
    star: wrap('<path d="m12 3 2.9 5.9 6.5.9-4.7 4.6 1.1 6.5L12 17.8 6.2 20.9l1.1-6.5L2.6 9.8l6.5-.9z"/>', { fill: 'currentColor' }),
    volume: wrap('<path d="M11 5 6 9H2v6h4l5 4V5z"/><path d="M15.5 8.5a5 5 0 0 1 0 7M19 5a9 9 0 0 1 0 14"/>'),
    micOff: wrap('<path d="M12 2a3 3 0 0 1 3 3v5m-6 0V5a3 3 0 0 1 .5-1.7"/><path d="M19 10a7 7 0 0 1-10.6 6M5 10a7 7 0 0 0 2 4.9"/><path d="M12 19v3M3 3l18 18"/>'),
    clock: wrap('<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/>'),
    video: wrap('<path d="m23 7-7 5 7 5V7z"/><rect x="1" y="5" width="15" height="14" rx="2"/>'),
    phone: wrap('<path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3.1 19.5 19.5 0 0 1-6-6A19.8 19.8 0 0 1 2.1 4.2 2 2 0 0 1 4.1 2h3a2 2 0 0 1 2 1.7c.1 1 .4 1.9.7 2.8a2 2 0 0 1-.5 2.1L8.1 9.9a16 16 0 0 0 6 6l1.3-1.3a2 2 0 0 1 2.1-.4c.9.3 1.8.6 2.8.7a2 2 0 0 1 1.7 2z"/>'),
    send: wrap('<path d="M22 2 11 13"/><path d="M22 2 15 22l-4-9-9-4 20-7z"/>'),
    mic: wrap('<rect x="9" y="2" width="6" height="11" rx="3"/><path d="M19 10a7 7 0 0 1-14 0M12 17v4"/>'),
    image: wrap('<rect x="3" y="3" width="18" height="18" rx="2.5"/><circle cx="8.5" cy="8.5" r="1.6"/><path d="m21 15-5-5L5 21"/>'),
    settings: wrap('<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.7 1.7 0 0 0 .3 1.8l.1.1a2 2 0 1 1-2.8 2.8l-.1-.1a1.7 1.7 0 0 0-2.9 1.2v.2a2 2 0 1 1-4 0v-.1a1.7 1.7 0 0 0-1.1-1.5 1.7 1.7 0 0 0-1.9.3l-.1.1a2 2 0 1 1-2.8-2.8l.1-.1a1.7 1.7 0 0 0-1.2-2.9H3a2 2 0 1 1 0-4h.1a1.7 1.7 0 0 0 1.2-2.9l-.1-.1a2 2 0 1 1 2.8-2.8l.1.1a1.7 1.7 0 0 0 1.9.3A1.7 1.7 0 0 0 10 3.1V3a2 2 0 1 1 4 0v.1a1.7 1.7 0 0 0 2.9 1.2l.1-.1a2 2 0 1 1 2.8 2.8l-.1.1a1.7 1.7 0 0 0 1.2 2.9h.2a2 2 0 1 1 0 4h-.2a1.7 1.7 0 0 0-1.5 1z"/>'),
    bell: wrap('<path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/>'),
    lock: wrap('<rect x="3" y="11" width="18" height="10" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>'),
    moon: wrap('<path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"/>'),
    logout: wrap('<path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><path d="m16 17 5-5-5-5M21 12H9"/>'),
    check: wrap('<path d="m20 6-11 11-5-5"/>'),
    checkDouble: wrap('<path d="m18 6-9.5 9.5L4 11"/><path d="m22 8-8 8"/>'),
    mute: wrap('<path d="M11 5 6 9H2v6h4l5 4V5z"/><path d="m23 9-6 6M17 9l6 6"/>'),
    chevron: wrap('<path d="m9 18 6-6-6-6"/>'),
    trash: wrap('<path d="M3 6h18M8 6V4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/>'),
    edit: wrap('<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.1 2.1 0 0 1 3 3L12 15l-4 1 1-4z"/>'),
    heart: wrap('<path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1-1.1a5.5 5.5 0 0 0-7.8 7.8l1.1 1L12 21l7.7-7.6 1.1-1a5.5 5.5 0 0 0 0-7.8z"/>'),
    switchCam: wrap('<path d="M21 2v6h-6"/><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M3 22v-6h6"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/>'),
    flash: wrap('<path d="M13 2 3 14h8l-1 8 10-12h-8l1-8z"/>'),
    /* Handbuch-Abgleich 01.09.2026: Datei, Gif, Sticker, Umfrage, Antwort,
       Zitat, Weiterleiten, Bearbeiten, Standortanfrage. */
    document: wrap('<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M9 13h6M9 17h6"/>'),
    film: wrap('<rect x="2" y="4" width="20" height="16" rx="2"/><path d="M7 4v16M17 4v16M2 10h20M2 14h20"/>'),
    sticker: wrap('<path d="M12 2a10 10 0 1 0 10 10h-6a4 4 0 0 1-4-4z"/><circle cx="9" cy="9" r="1"/><circle cx="15" cy="8" r="1"/>'),
    poll: wrap('<path d="M5 20V10M12 20V4M19 20v-6"/>'),
    antwort: wrap('<path d="M9 14 4 9l5-5"/><path d="M4 9h10a6 6 0 0 1 6 6v5"/>'),
    zitat: wrap('<path d="M7 7H4v6h6V7a4 4 0 0 1-3 4"/><path d="M17 7h-3v6h6V7a4 4 0 0 1-3 4"/>'),
    weiterleiten: wrap('<path d="m15 14 5-5-5-5"/><path d="M20 9H10a6 6 0 0 0-6 6v5"/>'),
    bearbeiten: wrap('<path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4z"/>'),
    standortAnfrage: wrap('<path d="M12 21s7-5.5 7-11a7 7 0 1 0-14 0c0 5.5 7 11 7 11z"/><path d="M10.5 9a1.5 1.5 0 1 1 2.4 1.2c-.6.4-.9.8-.9 1.3"/><path d="M12 14h.01"/>'),
    info: wrap('<circle cx="12" cy="12" r="9"/><path d="M12 16v-4M12 8h.01"/>'),
    block: wrap('<circle cx="12" cy="12" r="9"/><path d="m5.6 5.6 12.8 12.8"/>'),
    bookmark: wrap('<path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/>'),
    home: wrap('<path d="m3 10 9-7 9 7v9.5a1.5 1.5 0 0 1-1.5 1.5h-15A1.5 1.5 0 0 1 3 19.5z"/><path d="M9.5 21v-7h5v7"/>'),
    mapPin: wrap('<path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0z"/><circle cx="12" cy="10" r="3"/>'),
    grid: wrap('<rect x="3" y="3" width="7.5" height="7.5" rx="1.6"/><rect x="13.5" y="3" width="7.5" height="7.5" rx="1.6"/><rect x="3" y="13.5" width="7.5" height="7.5" rx="1.6"/><rect x="13.5" y="13.5" width="7.5" height="7.5" rx="1.6"/>'),
    portrait: wrap('<rect x="6" y="2" width="12" height="20" rx="2.5"/><path d="M10.5 18.5h3"/>'),
    landscape: wrap('<rect x="2" y="4" width="20" height="14" rx="2.5"/><path d="M8 22h8"/><path d="m10.5 9 4 2.2-4 2.2z"/>'),
    hash: wrap('<path d="M4 9h16M4 15h16M10 3 8 21M16 3l-2 18"/>'),
    music: wrap('<path d="M9 18V5l11-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="17" cy="16" r="3"/>'),
    userPlus: wrap('<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M19 8v6M22 11h-6"/>'),
    eye: wrap('<path d="M2 12s3.6-7 10-7 10 7 10 7-3.6 7-10 7-10-7-10-7z"/><circle cx="12" cy="12" r="3"/>'),
    compass: wrap('<circle cx="12" cy="12" r="9"/><path d="m15.5 8.5-2 5.5-5.5 2 2-5.5z"/>'),
    folder: wrap('<path d="M3 7.5A1.5 1.5 0 0 1 4.5 6h4L11 8.5h8.5A1.5 1.5 0 0 1 21 10v8a1.5 1.5 0 0 1-1.5 1.5h-15A1.5 1.5 0 0 1 3 18z"/>'),
    shield: wrap('<path d="M12 2 4 5.5v6c0 5 3.4 8.9 8 10.5 4.6-1.6 8-5.5 8-10.5v-6z"/>'),
    globe: wrap('<circle cx="12" cy="12" r="9"/><path d="M3 12h18"/><path d="M12 3a14 14 0 0 1 0 18 14 14 0 0 1 0-18z"/>'),
    repeat: wrap('<path d="m17 1 4 4-4 4"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><path d="m7 23-4-4 4-4"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/>'),
    /*
     * Durchgestrichene Glocke. Henrik am 26.08.2026, Punkt 22: "Nur Grau,
     * kein Strich wenn deaktiviert." Vorher wurde die normale Glocke mit
     * CSS durchgestrichen - text-decoration wirkt aber auf Text und nicht
     * auf ein SVG, also war da schlicht nie ein Strich.
     */
    // Glocke aus: dieselbe Glocke wie `bell`, der Strich 45° genau durch die
    // Mitte und nicht länger als die Glocke breit ist. Die Farben kommen aus
    // dem CSS (.ico-strich rot, .ico-strich-luecke in der Hintergrundfarbe),
    // sonst ist der Strich so grau wie die Glocke - Henrik, 21.09.2026.
    bellOff: wrap('<path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.7 21a2 2 0 0 1-3.4 0"/><path class="ico-strich-luecke" d="M5.2 5.2l13.6 13.6"/><path class="ico-strich" d="M5.2 5.2l13.6 13.6"/>'),
    // Das "..." aus dem Prototyp-Frame "CH + Kanal". Als gefuellte Kreise,
    // nicht als drei Punkte im Text - Satzzeichen sitzen auf der Grundlinie
    // und stehen dadurch nie mittig in einem Knopf.
    /* Drei-Punkte-Menue am Beitrag (openBeitragOptionen). */
    link: wrap('<path d="M10 13a5 5 0 0 0 7.5.5l3-3a5 5 0 0 0-7-7l-1.7 1.7"/><path d="M14 11a5 5 0 0 0-7.5-.5l-3 3a5 5 0 0 0 7 7l1.7-1.7"/>'),
    download: wrap('<path d="M12 3v12"/><path d="m7 10 5 5 5-5"/><path d="M5 21h14"/>'),
    eyeOff: wrap('<path d="M17.9 17.9A10.4 10.4 0 0 1 12 19c-6.4 0-10-7-10-7a18.5 18.5 0 0 1 5.1-5.9M9.9 5.2A9.1 9.1 0 0 1 12 5c6.4 0 10 7 10 7a18.6 18.6 0 0 1-2.2 3.2M14.1 14.1a3 3 0 1 1-4.2-4.2"/><path d="m2 2 20 20"/>'),
    flag: wrap('<path d="M4 22V4"/><path d="M4 4h13l-2 4.5L17 13H4"/>'),
    dots: wrap('<circle cx="5" cy="12" r="1.6" fill="currentColor" stroke="none"/><circle cx="12" cy="12" r="1.6" fill="currentColor" stroke="none"/><circle cx="19" cy="12" r="1.6" fill="currentColor" stroke="none"/>'),
  };
})();
